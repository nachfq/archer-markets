...
type Props = {
  account?: string; positions: Position[]; bids: Bid[]; now: bigint; selected?: Position; symbol: string;
  ...
  onDepth?: (kind: 0 | 1, strikeTotal: bigint, expiry: bigint) => Promise<{ positions: Position[]; bids: Bid[] }>;
};

export default function OptionsChain(props: Props) {
  const { positions, bids, now, symbol, quoteSymbol, underlyingDecimals, quoteDecimals } = props;
  const [chosenExpiry, setChosenExpiry] = useState<bigint | null>(null);
  const [mobileSide, setMobileSide] = useState("calls");
  const [expanded, setExpanded] = useState<string | null>(null);
  const [depth, setDepth] = useState<{ key: string; loading: boolean; positions: Position[]; bids: Bid[]; error?: string }>();
  const depthRequest = useRef(0);
  const dates = bookExpirations(positions, bids, now);
  ...
  const rows = expiry ? orderBook(positions, bids, expiry, now) : [];
  ...
  function quoteButton(quote: Position | Bid | undefined, bid: boolean, count?: bigint) {
    ...
    return <button key={key} className={`book-quote ${bid ? "bid" : "ask"} ${mine ? "own-quote" : ""}`}
      ...
      data-bid={bid ? key : undefined} data-offer={bid ? undefined : key}
  ...
  async function toggleDepth(row: (typeof rows)[number], series: Omit<OrderSeed, "side">) {
    ...
    if (!props.onDepth || !expiry) return;
    const request = ++depthRequest.current;
    const strikeTotal = row.numerator * 10n ** BigInt(underlyingDecimals) / row.denominator;
    setDepth({ key: row.key, loading: true, positions: [], bids: [] });
    try {
      const pages = await Promise.all(([0, 1] as const).map(kind => props.onDepth!(kind, strikeTotal, expiry)));
      if (depthRequest.current === request) setDepth({ key: row.key, loading: false, positions: pages.flatMap(page => page.positions), bids: pages.flatMap(page => page.bids) });
    } catch {
      if (depthRequest.current === request) setDepth({ key: row.key, loading: false, positions: [], bids: [], error: "Could not load depth. Close and retry." });
    }
  }
  return <section className="chain-panel" aria-label="Options chain">
    ...
    <div className={`chain-table-wrap show-${mobileSide}`}><table className="chain-table bid-ask-table">
      ...
      <tbody>{rows.map(row => {
        ...
        const series = { expiry, strike: unitInput(row.numerator, row.denominator, underlyingDecimals, quoteDecimals).value };
        const loaded = depth?.key === row.key && !depth.loading ? orderBook(depth.positions, depth.bids, expiry!, now).find(item => item.key === row.key) : undefined;
