  ...
  return <section className="chain-panel" aria-label="Options chain">
    ...
    <div className={`chain-table-wrap show-${mobileSide}`}><table className="chain-table bid-ask-table">
      ...
      <tbody>{rows.map(row => {
        ...
        return <Fragment key={row.key}><tr><td className="call-side">{side(row.calls)}</td><th scope="row" className="strike-cell"><button aria-label={`Strike ${price(row.numerator, row.denominator)}`} aria-expanded={opened} onClick={() => { setExpanded(opened ? null : row.key); props.onContext(series); }}>{price(row.numerator, row.denominator)}<small>{opened ? "Hide orders" : "All orders"}</small></button></th><td className="put-side">{side(row.puts)}</td></tr>
          {opened && <tr className="book-depth"><td className="call-side">{side(row.calls, true)}</td><td className="strike-cell"><small>New order<br />{([0, 1] as const).map(kind => <span key={kind}>{kind === 0 ? "Call" : "Put"}{(["buy", "sell"] as const).map(action => <button key={action} className="text-button" disabled={props.disabled} onClick={() => props.onCreate(action, { ...series, kind })}>{action === "buy" ? "Buy" : "Sell"}</button>)}</span>)}</small></td><td className="put-side">{side(row.puts, true)}</td></tr>}
        </Fragment>;
      })}</tbody>
    </table></div>
