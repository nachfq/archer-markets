...
export default function BuyRequests(props: Props) {
  ...
  return <TradeTicket title="Manage your bid" busy={busy} onClose={props.onClose}><div className="request-review">
    ...
    {selected && own ? <><h2>{selected.optionType === 0 ? "Call" : "Put"} · {units(selected.underlyingAmount, net.underlying.decimals)} {net.underlying.symbol}</h2>
      <dl><div><dt>{selected.state === 1 ? "Premium paid" : selected.state === 2 ? "Premium returned" : "Reserved premium"}</dt><dd>{units(selected.premium, net.quote.decimals)} {net.quote.symbol}</dd></div><div><dt>Exercise payment · total</dt><dd>{units(selected.strikeTotal, net.quote.decimals)} {net.quote.symbol}</dd></div>{net.version !== 4 && <div><dt>Accept before</dt><dd>{utcDeadline(selected.acceptUntil)}</dd></div>}<div><dt>Exercise before</dt><dd>{utcDeadline(selected.expiry)}</dd></div></dl>
      ...
      {selected.state === 1 ? <button className="button" disabled={busy} onClick={() => props.onOption(selected.option)}>View option</button> : selected.state === 2 ? <p>Bid canceled.</p> : <><p>{selected.acceptUntil <= now ? "The bid has expired. Recover its reserved premium." : "Cancel before execution to recover the full premium."}</p><button className="button dark" disabled={!props.canAct || props.unavailable} onClick={() => props.onRun(() => prepareCancelRequest(client, asMarket(net)!, account!, selected.id), "Bid canceled. The full premium was returned to your wallet.")}>Cancel bid & recover premium</button></>}
