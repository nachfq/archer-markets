...
import { prepareCancelV4, type Bid, type PreparedOperation } from "@stock-options-lab/sdk";
...
export default function BidManager(props: Props) {
  ...
  return <TradeTicket title="Manage your bid" busy={busy} onClose={props.onClose}><div className="bid-review">
    ...
    {selected && own ? <><h2>{selected.optionType === 0 ? "Call" : "Put"} · {units(selected.underlyingAmount, net.underlying.decimals)} {net.underlying.symbol}</h2>
      <dl><div><dt>{selected.state === 1 ? "Premium paid" : selected.state === 2 ? "Premium returned" : "Reserved premium"}</dt><dd>{units(selected.premium, net.quote.decimals)} {net.quote.symbol}</dd></div><div><dt>Exercise payment · total</dt><dd>{units(selected.strikeTotal, net.quote.decimals)} {net.quote.symbol}</dd></div><div><dt>Exercise before</dt><dd>{utcDeadline(selected.expiry)}</dd></div></dl>
