...
export default function OrderTicketV4(props: OrderTicketProps) {
    ...
    <form onSubmit={e => { e.preventDefault(); if (valid)
      ...
      <div className="order-execution"><strong>{crosses ? 'Estimated: executes now' : 'Estimated: open order'}</strong></div>
      <dl className="order-totals"><div><dt>{side === 'buy' ? 'Limit premium' : 'Asking premium'}</dt><dd>{fmt(terms?.premium)}</dd></div>{crosses && <div><dt>Matching premium</dt><dd>{fmt(bestPrice)}</dd></div>}{side === 'buy' && <div><dt>{crosses ? 'Trading fee' : 'Fee reserved'}</dt><dd>{fmt(crosses ? executionFee : maximumFee)}</dd></div>}<div><dt>{resale ? 'Additional collateral' : side === 'buy' ? 'Maximum to reserve' : 'Collateral to deposit'}</dt><dd>{fmt(funding, token.symbol, token.decimals)}</dd></div><div><dt>Available</dt><dd>{account ? fmt(available, token.symbol, token.decimals) : 'Connect wallet'}</dd></div></dl>
      <p className="fine">Exercise later requires {kind === 0 ? fmt(terms?.strikeTotal) : `1 ${net.underlying.symbol}`} from the holder before expiration.</p>
      {props.notification}
