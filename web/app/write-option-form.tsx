...
export default function WriteOptionForm({ requestMode = false, values, onChange, underlying, quote, busy, canMax, canReview, effectiveExpiry, suggestions, onMax, onReview, children }: Props) {
  return <form onSubmit={onReview}>
    <div className="section-head"><div><div className="eyebrow">{requestMode ? "BUY TO OPEN" : "SELL TO OPEN"}</div><h2>{requestMode ? "Post a bid" : "Post an ask"}</h2></div><span className="pill">{requestMode ? "Premium reserved" : "Fully collateralized"}</span></div>
    <div className="write-fields">
      <label className="field">Option type<select aria-label="Write option type" value={values.kind} onChange={event => onChange.kind(Number(event.target.value))} disabled={busy}><option value="0">Call</option><option value="1">Put</option></select><small>{requestMode ? `You reserve the premium in ${quote.symbol}.` : `You deposit ${values.kind === 0 ? underlying.symbol : quote.symbol}.`}</small></label>
    ...
    {children}
    <div className="form-actions"><small>{requestMode ? "Your bid waits for a seller. Cancel before acceptance to recover the premium." : "Your ask waits for a buyer. Premium arrives only when they accept."}</small><button className="button dark" type="submit" disabled={!canReview || busy}>{requestMode ? "Review buy order →" : "Review sell order →"}</button></div>
