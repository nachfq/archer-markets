...
const market = { id: 'one', chainId: 31337, factory: addr(5), deploymentBlock: 1n, version: 4, underlying: stock, quote: usd, sandbox: true };
...
test('portfolio separates V4 bid reserves, writer collateral and reclaimable funds', () => {
  ...
  assert.deepEqual(rows[1], { token: usd, available: 19n, openBidPremium: 7n, refundableBidPremium: 5n, openCollateral: 7n, activeCollateral: 0n, reclaimable: 0n, totalTracked: 38n });
});
...
test('duplicate snapshots do not double-count options or bids', () => {
  ...
  assert.equal(rows[1].totalTracked, 7n);
});
