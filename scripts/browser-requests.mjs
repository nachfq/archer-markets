...
try {
  ...
  for (const scenario of [0,1,2,3]) {
    ...
    if (scenario === 0) {
      ...
      await expect(buyer.getByRole('button', {name:'Reserve premium & request option'})).toHaveCount(0);
    ...
    if (scenario >= 2) {
      if (scenario === 3) {
        ...
        await expect(buyer.locator(`[data-request="${id}"]`)).toContainText('Premium recoverable');
      }
    ...
    if (scenario === 0) {
      ...
      await expect(writer.getByRole('button',{name:'Accept & write option',exact:true})).toBeDisabled();
      await writer.context().unroute(`${rpc}/**`);
      await writer.getByRole('button',{name:'Retry connection',exact:true}).click();
      await expect(writer.getByRole('button',{name:'Accept & write option',exact:true})).toBeEnabled();
    }
