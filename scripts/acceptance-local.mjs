...
async function ready(url, child, rpc = false) {
  for (let i = 0; i < 120; i++) {
    ...
    try {
      const response = await fetch(url, { ...(rpc ? { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'eth_chainId', params: [] }) } : {}), signal: AbortSignal.timeout(2000) });
